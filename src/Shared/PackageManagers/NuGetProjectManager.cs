...
namespace Microsoft.CST.OpenSource.PackageManagers
{
    ...
    public abstract class NuGetProjectManager : TypedManager<NuGetPackageVersionMetadata, NuGetProjectManager.NuGetArtifactType>
    {
        /// <summary>
        ...
        protected NuGetProjectManager(IManagerPackageActions<NuGetPackageVersionMetadata> actions, IHttpClientFactory httpClientFactory, string directory, TimeSpan? timeout = null) : base(actions, httpClientFactory, directory, timeout)
        {
        }
        ...
        internal static NuGetProjectManager Create(string destinationDirectory, IHttpClientFactory httpClientFactory, TimeSpan? timeout, PackageURL? packageUrl)
        {
            if (packageUrl is null || packageUrl.Qualifiers?.TryGetValue("repository_url", out string? repositoryUrlQualifier) != true ||
                repositoryUrlQualifier?.TrimEnd('/') == NuGetProjectManagerV3.NUGET_DEFAULT_INDEX)
            {
                return new NuGetProjectManagerV3(destinationDirectory, NuGetPackageActions.CreateV3(), httpClientFactory, timeout);
            }
            else if (repositoryUrlQualifier?.TrimEnd('/') == NuGetProjectManagerV2.POWER_SHELL_GALLERY_DEFAULT_INDEX)
            {
                return new NuGetProjectManagerV2(destinationDirectory, NuGetPackageActions.CreateV2(), httpClientFactory, timeout);
            }
            else
            {
                throw new NotImplementedException($"NuGet package URLs having a repository URL other than '{NuGetProjectManagerV2.POWER_SHELL_GALLERY_DEFAULT_INDEX}' or '{NuGetProjectManagerV3.NUGET_DEFAULT_INDEX}' are not currently supported.");
            }
        }
