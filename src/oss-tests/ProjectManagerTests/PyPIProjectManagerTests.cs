...
public class PyPIProjectManagerTests
{
    ...
    [Theory]
    [InlineData("pkg:pypi/pandas@1.4.2", "Powerful data structures for data analysis, time series, and statistics")]
    [InlineData("pkg:pypi/plotly@5.7.0", "An open-source, interactive data visualization library for Python")]
    [InlineData("pkg:pypi/requests@2.27.1", "Python HTTP for Humans.")]
    ...
    [Theory]
    [InlineData("pkg:pypi/pandas@1.4.2", "2022-04-02T10:32:27")]
    [InlineData("pkg:pypi/plotly@5.7.0", "2022-04-05T16:26:03")]
    [InlineData("pkg:pypi/requests@2.27.1", "2022-01-05T15:40:49")]
    ...
    [Theory]
    [InlineData("pkg:pypi/pandas@1.4.2", 86, "1.4.2")]
    [InlineData("pkg:pypi/plotly@3.7.1", 276, "5.7.0")]
    [InlineData("pkg:pypi/requests@2.27.1", 145, "2.27.1")]
    ...
    [Theory]
    [InlineData("pkg:pypi/pandas@1.4.2")]
    [InlineData("pkg:pypi/plotly@3.7.1")]
    [InlineData("pkg:pypi/requests@2.27.1")]
    public async Task PackageVersionExistsAsyncSucceeds(string purlString)
    {
        PackageURL purl = new(purlString);

        Assert.True(await _projectManager.PackageVersionExistsAsync(purl, useCache: false));
    }
    ...
    public async Task GetPublishedAtUtcSucceeds(string purlString, string? expectedTime = null)
    {
        ...
        {
            Assert.Equal(DateTime.Parse(expectedTime).ToUniversalTime(), time);
        }
    }

    [Theory]
    [InlineData("pkg:pypi/pandas", true)]
    [InlineData("pkg:pypi/plotly@3.7.1", true)]
    [InlineData("pkg:pypi/notarealpackage", false)]
    ...
    [Theory]
    [InlineData("pkg:pypi/pandas@1.4.2", true)]
    [InlineData("pkg:pypi/pandas@12.34.56.78", false)]
    [InlineData("pkg:pypi/plotly@5.7.0", true)]
    [InlineData("pkg:pypi/requests@2.27.1", true)]
    [InlineData("pkg:pypi/notarealpackage@0.0.0", false)]
    ...
    [Theory]
    [InlineData("pkg:pypi/pandas@1.4.2")]
    [InlineData("pkg:pypi/plotly@5.7.0")]
    [InlineData("pkg:pypi/requests@2.27.1")]
    ...
    [Theory]
    [InlineData("microsoft", "pkg:pypi/azdev")]
