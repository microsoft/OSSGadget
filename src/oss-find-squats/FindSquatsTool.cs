                ...
                if (purl.Name is not null && purl.Type is not null)
                {
                    ...
                    if (manager is not null)
                    {
                        ...
                        foreach ((var candidate, var rules) in mutationsDict)
                        {
                            ...
                            var innerPurl = new PackageURL(purl.Type, candidate);
                            var versions = await manager.EnumerateVersions(innerPurl);

                            if (versions.Any())
                            {
                                Logger.Warn($"{candidate} package exists. Potential squat.");
                                ...
                                var message = $"Potential Squat candidate {candidate} detected. Generated by {rls}";
                                res.Message = new Message(message, null, rls, null, null);
