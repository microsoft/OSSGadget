...
namespace Microsoft.CST.OpenSource.Tests.ProjectManagerTests
{
    ...
    public class NuGetProjectManagerTests
    {
        ...
        private readonly Mock<INuGetProvider> _mockNuGetProvider = new();
        ...
        /// <summary>
        /// Set up the <see cref="NuGetProjectManager"/> for this test run.
        /// </summary>
        /// <param name="purl">The <see cref="PackageURL"/> to use when configuring the mocked calls for this manager.</param>
        private void SetupManager(PackageURL purl)
        {
            _mockNuGetProvider.Setup(provider => provider.GetMetadataAsync(It.Is<PackageURL>(p => p.Name.Equals(purl.Name) && (purl.Version == null || p.Version.Equals(purl.Version))), It.IsAny<bool>(), It.IsAny<CancellationToken>()).Result).Returns(
                NuGetMetadata.FromJson(_metadata[purl.ToString()]));
            
            _mockNuGetProvider.Setup(provider => provider.GetAllVersionsAsync(It.Is<PackageURL>(p => p.Name.Equals(purl.Name)), It.IsAny<bool>(), It.IsAny<CancellationToken>()).Result).Returns(
                JsonConvert.DeserializeObject<IEnumerable<string>>(_versions[purl.ToString()]) ?? Array.Empty<string>());
            
            _projectManager = new NuGetProjectManager(_mockFactory.Object, ".", _mockNuGetProvider.Object);
        }
